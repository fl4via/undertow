...
public class Cookies {
    ...
    public static void parseRequestCookies(int maxCookies, boolean allowEqualInValue, List<String> cookies, Set<Cookie> parsedCookies) {
        Map parsedCookiesMap = parseRequestCookies(maxCookies, allowEqualInValue, cookies, LegacyCookieSupport.COMMA_IS_SEPARATOR, LegacyCookieSupport.ALLOW_HTTP_SEPARATORS_IN_V0);
        parsedCookies.addAll(parsedCookiesMap.values());
    }
    ...
    static Map<String, Cookie> parseRequestCookies(int maxCookies, boolean allowEqualInValue, List<String> cookies, boolean commaIsSeperator, boolean allowHttpSepartorsV0) {
        return parseRequestCookies(maxCookies, allowEqualInValue, cookies, commaIsSeperator, allowHttpSepartorsV0, false);
    }

    @SuppressWarnings("removal")
    private static Map<String, Cookie> parseRequestCookies(int maxCookies, boolean allowEqualInValue, List<String> cookies, boolean commaIsSeperator, boolean allowHttpSeparatorsV0, final boolean rfc6265CookieValidationEnabled) {
        if (cookies == null) {
            return new TreeMap<>();
        }
    ...
    public static Map<String, Cookie> parseRequestCookies(List<String> cookies, CookieStore requestCookieStore, OptionMap options) {
        return parseRequestCookies(cookies, requestCookieStore, options, LegacyCookieSupport.COMMA_IS_SEPARATOR, LegacyCookieSupport.ALLOW_HTTP_SEPARATORS_IN_V0);
    }

    static Map<String, Cookie> parseRequestCookies(List<String> cookies, CookieStore requestCookieStore, OptionMap options, boolean commaIsSeparator, boolean allowHttpSeparatorsInV0) {
        if (cookies == null) {
            return new TreeMap<>();
        }
        final int maxCookies;
        final boolean allowEqualInValue;
        final boolean enableRfc6265CookieValidation;
        if (options == null || options == OptionMap.EMPTY) {
            maxCookies = UndertowOptions.DEFAULT_MAX_COOKIES;
            allowEqualInValue = false;
            enableRfc6265CookieValidation = UndertowOptions.DEFAULT_ENABLE_RFC6265_COOKIE_VALIDATION;
        }
        else {
            maxCookies = options.get(UndertowOptions.MAX_COOKIES, UndertowOptions.DEFAULT_MAX_COOKIES);
            allowEqualInValue = options.get(UndertowOptions.ALLOW_EQUALS_IN_COOKIE_VALUE, false);
            enableRfc6265CookieValidation = options.get(UndertowOptions.ENABLE_RFC6265_COOKIE_VALIDATION, UndertowOptions.DEFAULT_ENABLE_RFC6265_COOKIE_VALIDATION);
        }
        for (String cookie : cookies) {
            parseCookie(cookie, requestCookieStore, maxCookies, allowEqualInValue, commaIsSeparator, allowHttpSeparatorsInV0, enableRfc6265CookieValidation);
        }
        return requestCookieStore.asLegacyMap();
    }

    private static void parseCookie(final String cookie, final CookieStore parsedCookies, final int maxCookies, final boolean allowEqualInValue, final boolean commaIsSeperator, final boolean allowHttpSepartorsV0, final boolean rfc6265CookieValidationEnabled) {
    ...
    private static void createCookie(final String value, final CookieJar cookieJar) {
        ...
        if (!cookieJar.name.isEmpty() && cookieJar.name.charAt(0) == '$') {
            if (cookieJar.name.equals(VERSION)) {
                cookieJar.version = Integer.parseInt(stripQuotes(value));
                //Theoretically this should happen only once at the start
                applyAdditional(cookieJar, cookieJar.name, stripQuotes(value));
            } else {
                applyAdditional(cookieJar, cookieJar.name, stripQuotes(value));
            }
        } else {
    ...
    private static void applyAdditional( final CookieJar cookieJar, final String name, final String value) {
        ...
        if (cookieJar.version == 1) {
            ...
            if (cookieJar.currentCookie != null) {
                ...
                switch(name) {
                    case DOMAIN:
                        cookieJar.currentCookie.setDomain(stripQuotes(value));
                        break;
                    case PATH:
                        cookieJar.currentCookie.setPath(stripQuotes(value));
                }
            } else {
                switch(name) {
                    case DOMAIN:
                        cookieJar.currentDomain = stripQuotes(value);
                        break;
                    case PATH:
                        cookieJar.currentPath = stripQuotes(value);
                }
            }
        }
    }
